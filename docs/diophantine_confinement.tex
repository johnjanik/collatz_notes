...
%------------------------------------------------------------------
\title{Diophantine Confinement in Syracuse Dynamics}
...
reaching~$1$ implies the K-bound, so the sole critical-path sorry
is formally equivalent to the conjecture itself.  A Syracuse framework
bridge connects Tao's odd-step formulation to the walk/drift
infrastructure.  The argument is formalized in approximately
9\,040~lines of Lean~4 across 36~files with five external axioms
(Baker~1966, Rhin~1987, Hercher~2023, Weyl~1916,
Baker--Steiner~2024), all citing published theorems.  A skew product decomposition identifies the
Collatz dynamics on the $(2,3)$-solenoid with an irrational rotation
cocycle over the $2$-adic odometer; the cocycle mean irrationality and
correlation decay are proved, but the gap between ergodicity and
bounded deficit remains open.
\end{abstract}
...
$3\nu_3(t) \le t + K$ is the sole critical sorry
(\texttt{finite\_deficit\_bound} in \texttt{DiophantineRepeller.lean});
...
9\,040~lines across 36~files, building in 3\,112~jobs.
...
\begin{tabular}{lll}
  ...
  Cycle elim.\ ($\Delta_3 \ge 80$) & \texttt{SteinerCycle.lean} &
    Case split: $K \le 91$ (A3) $+$ $K \ge 92$ (A5) \\
  ...
  Cell error algebra & \texttt{SolenoidMixing.lean} &
    Cell error shift, Hensel--Baker conflict \\
  Correlation decay & \texttt{CorrelationDecay.lean} &
    Autocorrelation vanishing, deficit compensation \\
  Skew product & \texttt{SkewProduct.lean} &
    Cocycle tracking, fiber coordinate identification \\
  Borel--Cantelli & \texttt{BorelCantelli.lean} &
    Survivor density, danger density $\to$ 0 \\
...
\subsection{Remaining gaps: four axioms and the critical path}

Five external axiom declarations appear in the formalization.  All five
cite established results with published proofs; none represents
...
\begin{tabular}{clll}
  ...
  A5 & \texttt{SteinerCycle.lean} &
    \texttt{baker\_steiner\_no\_large\_cycle} &
    Extends A3 to $m \ge 92$ \\
  \bottomrule
\end{tabular}
\end{center}
...
The sole critical-path \texttt{sorry} is
\texttt{finite\_deficit\_bound} (\texttt{DiophantineRepeller.lean}):
for every $n \ge 1$, there exists $D \in \Z$ such that
$\mathrm{deficit}(n, t) \le D$ for all~$t$.
The proof chain is:
\[
  \texttt{finite\_deficit\_bound} \;\to\;
  \text{K-bound} \;\to\;
  \texttt{collatz\_conjecture}.
\]
All arrows are sorry-free (\texttt{DiophantineRepeller.lean},
...
\texttt{Conclusion.lean}), so bounded deficit is formally equivalent
to the Collatz conjecture.

\medskip\noindent\textbf{Note on the SlidingWindowCondition bug.}
An earlier version of the formalization used
\texttt{SlidingWindowCondition} (SWC: $\forall t,\;
\mathrm{deficit}(t + W) \le \mathrm{deficit}(t)$, i.e.,
deficit non-increasing over windows) as the critical-path target.
This condition is \emph{false} for many starting values: for $n = 27$,
the deficit climbs from~$0$ to~$12$ during the transient before
stabilizing in the $1 \to 4 \to 2 \to 1$ cycle.  Computationally,
${\sim}42.6\%$ of starting values up to $10^7$ violate SWC for all~$W$.
The corrected formulation (bounded deficit) avoids this error.

\medskip\noindent\textbf{Structural support: the skew product
decomposition.}
The Collatz map on the $2$-adic integers $\Z_2$ has a natural
\emph{skew product structure} (\texttt{SkewProduct.lean}):
\begin{itemize}
\item \emph{Base}: the $2$-adic odometer $\sigma \colon \Z_2 \to \Z_2$
  (adding~$1$), which is uniquely ergodic with respect to Haar measure.
\item \emph{Fiber}: the torus $\mathbb{T} = \R / \Z$.
\item \emph{Cocycle}: $\varphi(x) = \log_2 3$ if $x$ is odd, $0$ if
  $x$ is even.  The cocycle sum $\varphi_t = \log_2 3 \cdot \nu_3(t)$
  equals the walk/fiber coordinate (proved sorry-free).
\end{itemize}
The mean cocycle value is $\log_2 3 / 2$, which is irrational (proved
from \texttt{irrational\_logb\_two\_three}).  By Furstenberg's theorem
on skew products over uniquely ergodic bases with irrational fiber
rotation~\cite{Furstenberg1961}, the skew product is ergodic.
Ergodicity gives $\nu_3(t) / t \to 1/3$ (Birkhoff averages), but the
gap between ``time averages converge'' and ``deficit is bounded''
($\exists D,\; \forall t,\; \mathrm{deficit}(t) \le D$) remains
open---this is exactly the content of \texttt{finite\_deficit\_bound}.

Three proved forces provide structural evidence that the deficit
should be bounded:
\begin{enumerate}
...
\item \emph{Baker cell separation} (sorry-free, reduces to A1):
  ...
  $> C / \max(|a|, |b|)^{\kappa}$.
\item \emph{Correlation decay} (sorry-free, \texttt{CorrelationDecay.lean}):
  for $d \ge 2$ and danger threshold $\delta < 1/2$, the autocorrelation
  of the danger indicator vanishes \emph{exactly}: $C(d) = 0$.  The
  shifted danger sets are proved disjoint on the torus, since the
  cell error shift $|d \cdot (1 - \log_2 3)| > 1$ exceeds twice the
  threshold.
\end{enumerate}

\medskip\noindent\textbf{Additional \texttt{sorry} declarations.}
Beyond the axioms, six \texttt{sorry} declarations remain in the
formalization.  Three are equivalent to the Collatz conjecture
(\texttt{nu3\_linear\_bound} in \texttt{Drift.lean},
\texttt{finite\_deficit\_bound} in \texttt{DiophantineRepeller.lean},
and \texttt{equidistribution\_implies\_deficit\_bounded} in
\texttt{WeylEquidistribution.lean}).
Two are in \texttt{WeylEquidistribution.lean}: a solenoid bridge
(\texttt{cellSeqNu2\_equidistributed}, connecting Weyl's irrational
rotation to the actual Collatz cell visits---an ergodic-theoretic
statement, not Collatz-equivalent by itself) and a standard
combinatorics lemma (\texttt{equidistributed\_subset\_visits\_lower}).
One is a Blaschke product sub-lemma
(\texttt{poisson\_jensen\_blaschke} in
\texttt{GrowthEstimates.lean}), vestigial since Baker's theorem is
axiom~A1.

\medskip\noindent
Compared to the initial formalization, cycle elimination is now
complete: \texttt{steiner\_cycle\_large} ($\Delta_3 \ge 80$) was
closed via axiom~A5, and the Weyl assembly theorem
\texttt{nu3\_linear\_bound\_from\_weyl} was proved by decomposing it
into the solenoid bridge and deficit accounting sub-lemmas.
...
\subsection{The 100-billion run}

A complete run over all starting values $n \in [1, 10^{11}]$ was
performed (61.5~hours wall time, $25.1 \times 10^{12}$ total
Collatz steps, mean trajectory length $251.26$ steps).  The global
odd-step proportion is $p_{\mathrm{odd}} = 0.332473361$.
...
\begin{tabular}{lrl}
  ...
  \textbf{Quantity} & \textbf{Value} & \textbf{Note} \\
  \midrule
  Starting values $N$ & $9.9\!\times\!10^{10}$ & \\
  Total steps & $25.1 \times 10^{12}$ & \\
  Mean $p_{\mathrm{odd}}$ & $0.332473$ &
    Stable vs.\ $10^{10}$ run \\
  ``11'' violations & $0$ & Over $25.1 \times 10^{12}$ transitions \\
  Branch cells ($k \ge 216$) & $21{,}632$ &
    Unchanged since $N \approx 10^8$ \\
  Runtime & 61.5~h & 24-thread OpenMP \\
  \bottomrule
\end{tabular}
\end{center}

The most striking new feature at $10^{11}$ is a \emph{Diophantine
ghost island}: a cluster of $251$~cells at perpendicular distance
${\sim}370$ from the main branch locus at $k = 729$, aligned with the
best rational approximant $460/729 \approx \log_3 2$.  Between the
main cluster (perpendicular distance $|d| \le 18$) and the island
($d \approx 370$) lies a complete void of $352$ units---a geometric
shadow of the continued fraction expansion of $\log_2 3$.  The island
cells receive only $3{,}233$ total visits (fraction $1.3 \times
10^{-10}$) and all $251$ have zero oo~transitions, confirming the
golden mean shift at this extreme regime.

Of the $4{,}243$ newly occupied cells at $k = 729$ between $10^{10}$
and $10^{11}$, $251$ form the ghost island and $3{,}992$ extend the
main cluster boundary.  The branch cell count itself remains
frozen at $21{,}632$, unchanged from $N \approx 10^8$.  See the
companion paper~\cite{Janik2026figures} for detailed analysis
including cell statistics, shadow offset distributions, and the
downward-sloping foliation-position feature at $t \approx 235$.

\subsection{The equilibrium cusp}
\label{sec:cusp}

The branch locus computation records, for each torus cell
$(r_2, r_3) \in (\Z/k\Z)^2$, the fraction of odd-step visits
$p_{\mathrm{odd}}$ and the \emph{slope deviation}
\[
  \sigma(r_2, r_3) \;=\;
  \left|\frac{p_{\mathrm{odd}}}{1 - p_{\mathrm{odd}}} - \log_3 2\right|,
\]
the absolute deviation of the cell's odds ratio from the Anosov
equilibrium value $\log_3 2 = 1/\log_2 3$.  This quantity measures
how far the cell's local parity balance lies from the neutral drift
threshold.

Since $f(p) = p/(1-p)$ is smooth and strictly convex, $\sigma$
vanishes exactly at the equilibrium parity fraction
$p^* = 1/(1 + \log_2 3) \approx 0.38685$
(cf.\ Remark~\ref{rem:near-miss}), producing a sharp V-shaped cusp
in the $(p_{\mathrm{odd}},\, \sigma)$ plane.  The cusp is
\emph{asymmetric}: since $f'(p^*) = 1/(1-p^*)^2 = 2.660$ and
$f''(p^*) = 2/(1-p^*)^3 = 8.676$,
\begin{align*}
  \text{Left arm } (p < p^*)\colon \quad
    &\sigma \approx |\Delta p|\,(2.66 - 4.34\,|\Delta p|), \\
...
This reflects the multiplicative asymmetry of the Collatz map: the
growth factor $3/2$ per odd step exceeds the shrinkage factor $1/2$
per even step, so deviations toward more odd steps produce larger
slope deviations.

Power-law fits $\sigma \sim a\,|p - p^*|^\alpha$ over the branch
cells at $k = 216$ confirm $\alpha = 1.00 \pm 0.04$ on both arms
($R^2 > 0.9999$), with the small deviation from unity explained by
$f''(p^*)$ curvature.  The cusp appears at every $k$-level from
$k = 9$ onward, always at the same theoretical location~$p^*$, with
the minimum slope deviation improving from $0.031$ at $k = 9$ to
$3.85 \times 10^{-5}$ at $k = 216$.

Three observations from the cusp structure are significant for the
confinement argument:

\begin{enumerate}
\item \emph{Population asymmetry.}  The mean $p_{\mathrm{odd}}$ of
  branch cells at $k = 216$ is $0.284$, which is $0.103$ below~$p^*$.
  The vast majority of branch cells sit on the \emph{left}
  (convergent) arm of the cusp, where $\nu_2/\nu_3 > \log_2 3$.
  This is the population-level manifestation of the walk's positive
  drift (Section~\ref{sec:correction}).

\item \emph{The cusp is not at maximum entropy.}  The entropy at
  $p^*$ is $H(p^*) = 0.963$ ($96.3\%$ of the maximum $H = 1$ at
  $p = 1/2$).  The Collatz dynamics selects for drift neutrality,
  not maximum randomness.  The $11.3\%$ gap between $p^*$ and
  $p = 1/2$ quantifies the departure from a symmetric random walk.

\item \emph{Right-arm steepness and the golden mean gap.}  The
  steeper right arm means that cells exceeding $p^*$ incur a
  disproportionate slope penalty.  Combined with the golden mean
  shift constraint $p_{\mathrm{odd}} \le 1/\varphi^2 \approx 0.382
  < p^*$ (Proposition~\ref{prop:log23-lt-phi}), the SFT bound
  confines all Collatz parity sequences to the left arm.  The cusp
  geometry explains \emph{why} the $0.005$ gap between $1/\varphi^2$
  and $p^*$ is dynamically effective: the right arm's steepness
  amplifies even small excursions past equilibrium into large slope
  deviations, creating a soft barrier that reinforces the hard SFT
  bound.
\end{enumerate}
...
proof chain (Section~\ref{sec:lean}):
\[
\text{Baker separation} \;+\; \text{Hensel attrition}
\;\to\; [\text{bounded deficit: sorry}]
...
This requires showing that no trajectory can accumulate unbounded
deficit---a question about the \emph{mixing} properties of the
Collatz map on the $(2,3)$-solenoid, captured formally as the sorry
\texttt{finite\_deficit\_bound}.
...
All data and Lean 4 code can be found at https://github.com/johnjanik/syracuse-confinement
...
assistant infrastructure that made the formalization possible. During the preparation of this work,
the author used generative pre-trained transformers Claude Opus 4.6 to enhance readability and language,
aiding in formulating and structuring content. After using these tools, the authors reviewed and edited
...
\emph{Amer.\ J.\ Math.}
\textbf{83} (1961), no.~4, 573--601.
\newblock \href{https://doi.org/10.2307/2372899}%
  {\texttt{doi:10.2307/2372899}}.
...
\bibitem{Janik2026figures}
J.~A.~Janik,
On the Diophantine stability of the Syracuse map,
preprint, 2026.
