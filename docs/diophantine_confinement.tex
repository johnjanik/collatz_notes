...
\smallskip
\noindent
\emph{The tunnel wall approach and its limitation.}
...
``pure-even'' (visited only by even steps), and Baker's Diophantine
lower bound prevents this line from aligning with rational grid
lines.  If a trajectory visits these pure-even cells with positive
frequency, each such visit forces an even step, constraining
$\nu_3/t$ below $p_{\mathrm{eq}}$.

However, this approach encounters a fundamental obstacle: the
empirical definition of pure-even cells (based on observed trajectory
visits) does not survive cross-trajectory aggregation at small torus
scales.  Specifically, at scale $3^1$ (a $3 \times 3$ torus with 9
cells), examining trajectories from starting values $1$ through~10
over 20 time steps yields \emph{zero} pure-even cells---every cell
has been visited by at least one odd step from some trajectory
(Section~\ref{sec:tunnel-eval}).

For a single trajectory ($N = 1$), pure-even cells do persist: at
scale $3^1$, six of nine cells remain pure-even indefinitely.  This
is because the $1 \to 4 \to 2 \to 1$ cycle's odd visits trace a line
of slope~$2$ on the torus, and the Diophantine constraint
$2j \equiv 0, \; j \equiv 1 \pmod{3^b}$ is contradictory since
$\gcd(2, 3^b) = 1$.  But the multi-trajectory aggregation required
to connect Baker's theorem to arbitrary starting values $n$ remains
an open problem.
...
\cite{Wirsching1998}.  Alternatively, a \emph{structural} definition
of pure-even cells---one based on torus geometry and Baker's theorem
rather than on empirical visits---might overcome the cross-trajectory
contamination obstacle.  Tao's result~\cite{Tao2022}
...
exhibiting cell $(0,1)$ as pure-even for $N = 1$, $T = 1$ at any scale
$3^b$; \texttt{tunnel\_foliation\_intersection} was removed after its
statement was computationally disproved
(Section~\ref{sec:tunnel-eval}).
...
\subsection{Pure-even cell persistence}\label{sec:tunnel-eval}

The Lean~4 function \texttt{tunnelWallWidth}$(b, N, T)$ counts
pure-even cells on $(\Z/3^b\Z)^2$ for trajectories $1, \ldots, N$
over time steps $1, \ldots, T$.  The following data, obtained via
\texttt{\#eval}, reveal the cross-trajectory contamination effect:

\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{ccrl}
  \toprule
  \textbf{$b$} & \textbf{$N$} & \textbf{$T$}
  & \textbf{Pure-even cells} \\
  \midrule
  $1$ & $1$ & $1$ & $1$ \\
  $1$ & $1$ & $50$ & $6$ \quad (stable) \\
  $1$ & $10$ & $20$ & $0$ \quad (permanent) \\
  $2$ & $1$ & $1$ & $1$ \\
  $2$ & $10$ & $50$ & $19$ \\
  \bottomrule
\end{tabular}
...
For a single trajectory ($N = 1$), pure-even cells persist: six of
nine cells at scale $3^1$ are never visited by an odd step.  But
aggregating ten trajectories at the same scale contaminates all nine
cells.  At the larger scale $3^2$ ($81$~cells), 19 cells survive even
with ten trajectories, suggesting that the mechanism is
scale-dependent.

This data led to the removal of the
\texttt{tunnel\_foliation\_intersection} \texttt{sorry}, whose
statement (pure-even cells persist for all $b \ge 1$ and
arbitrarily many trajectories) was shown to be false at
$b = 1$, $N = 10$.
...
temporarily exceed $p_{\mathrm{eq}}$.  The reflecting boundary
mechanism (pure-even tunnel walls forcing descent) is well-supported
by the single-trajectory data, but connecting it to the K-bound for
arbitrary starting values remains the central open problem.
\end{remark}
