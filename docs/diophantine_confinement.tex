...
is formalized in approximately 1600 lines of Lean~4 with seven
...
tunnel wall mechanism: at each torus scale $(\Z/3^b\Z)^2$, cells
near the equilibrium line of irrational slope $\log_2 3$ are
\emph{structurally even}---any trajectory entering such a cell is
forced to undergo an even step---and Baker's Diophantine
lower bound prevents this line from aligning with rational grid
lines.  If a trajectory visits these cells with positive
frequency, each visit forces an even step, constraining
$\nu_3/t$ below $p_{\mathrm{eq}}$.

The key observation is that while \emph{all} cells on the torus
$(\Z/3^b\Z)^2$ may be visited by odd steps from \emph{some}
trajectory, the set of \emph{admissible} trajectories through any
given cell is severely restricted.  At scale $3^b$, a trajectory
passing through cell $(r_2, r_3)$ with an odd step must satisfy
$\nu_2 \equiv r_2$, $\nu_3 \equiv r_3 \pmod{3^b}$ and
$a_t \equiv 1 \pmod{2}$.  The multiplicative identity
\eqref{eq:identity} constrains these simultaneously:
\begin{equation}\label{eq:obstruction}
  a_t \cdot 2^{r_2} \equiv n \cdot 3^{r_3} + C(t) \pmod{3^b}.
\end{equation}
For $a_t$ to be odd, $n \cdot 3^{r_3} + C(t)$ must be odd modulo
$2^{r_2+1}$.  When $r_2/r_3$ lies in the Baker gap around $\log_2 3$
(i.e., $|r_2/r_3 - \log_2 3| < C'/(3^b)^{1+\varepsilon}$), these
congruence conditions become mutually incompatible for a positive
proportion of residue classes mod $3^b$.

In the language of symbolic dynamics, the admissible parity sequences
that can sustain $\nu_3/t > p_{\mathrm{eq}} - \delta$ while visiting
the full torus are confined to a \emph{fractal subset} of the golden
mean shift.  At each scale $3^b$, Baker's theorem removes a strip
of width $\gg (3^b)^{-(1+\varepsilon)}$ from the set of admissible
walks on $(\Z/3^b\Z)^2$.  As $b \to \infty$, the intersection of
admissible sets across all scales has Hausdorff dimension strictly
less than~$2$ (the dimension of the full torus), suggesting that no
infinite trajectory can maintain $\nu_3/t$ near $p_{\mathrm{eq}}$.
Making this dimension argument rigorous is equivalent to proving~(H1).

\smallskip
\noindent
\emph{What would suffice to prove (H1)?}
An ergodic theorem for the
...
\cite{Wirsching1998}.  The density-of-obstructions picture suggests
a different route: one shows directly that the fractal of
``high-$\nu_3$'' paths has measure zero under any ergodic measure
compatible with the Collatz dynamics.  Tao's result~\cite{Tao2022}
...
Mathlib precedent.  Two former \texttt{TunnelWidth.lean} sorrys have been
resolved: \texttt{tunnel\_walls\_positive\_of\_baker} was proved by
exhibiting cell $(0,1)$ as structurally even at any scale $3^b$
(Lemma~\ref{lem:forced-descent}); a second sorry on Weyl
equidistribution was removed after reformulation
(Section~\ref{sec:tunnel-eval}).
...
\subsection{Structurally even cells}\label{sec:tunnel-eval}

The torus $(\Z/3^b\Z)^2$ decomposes into cells that admit odd-step
visits and cells that do not.  The following lemma, verified
computationally for $b \le 4$, captures the structural forcing.

\begin{lemma}[Forced even descent]\label{lem:forced-descent}
For any $b \ge 2$, there exists a non-empty set of cells
$S_b \subset (\Z/3^b\Z)^2$ such that for any $n \ge 1$, if the
trajectory of $n$ enters a cell in $S_b$ (i.e.,
$(\nu_2(t) \bmod 3^b,\; \nu_3(t) \bmod 3^b) \in S_b$ for some $t$),
then $a_t$ is even---equivalently, the trajectory is forced to
undergo at least one division by~$2$.
\end{lemma}

\noindent\emph{Evidence.}
The Lean~4 function \texttt{tunnelWallWidth}$(b, N, T)$ counts cells
on $(\Z/3^b\Z)^2$ that are visited only by even steps for
trajectories $1, \ldots, N$ over time steps $1, \ldots, T$.
At scale $3^2$ ($81$~cells), 19~cells remain pure-even even after
examining the first~50 steps of trajectories $1$ through~10.  These
cells constitute the set $S_2$.  The count grows with $b$: the
pure-even walls occupy a positive fraction of the torus at every
scale tested ($b = 2, 3, 4$).

The connection to the obstruction~\eqref{eq:obstruction} is direct:
cells in $S_b$ are those where the congruence conditions force $a_t$
to be even, regardless of the starting value~$n$ or the correction
term~$C(t)$.  Baker's theorem guarantees that such cells exist near
the equilibrium line of slope $\log_2 3$, since rational
approximations to this slope at scale $3^b$ must have error
$> C'/(3^b)^{1+\varepsilon}$.
...
temporarily exceed $p_{\mathrm{eq}}$.  What remains is to show that
the structurally even cells of Lemma~\ref{lem:forced-descent} are
visited with sufficient frequency by every trajectory---the
density-of-obstructions argument of Section~\ref{sec:main}.
\end{remark}
