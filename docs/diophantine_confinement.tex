...
with cycle elimination---proved here for all cycle lengths, delegating
longer cycles ($\Delta_3 \ge 2$) to Baker--Steiner estimates---this
yields a conditional proof of the Collatz conjecture:
linear walk drift implies $\mathrm{collatzReaches}(n)$.  We also prove
the converse: reaching~$1$ implies the K-bound, so the sole
critical-path axiom is formally equivalent to the conjecture itself.
The argument is formalized in approximately 2800 lines of Lean~4 with
seven explicit \texttt{sorry} axioms, all identified and categorized.
\end{abstract}
...
by~(H2), Baker's theorem provides effective lower bounds on
$|2^{\Delta_2} - 3^{\Delta_3}|$ that, combined with Steiner-type
analysis of the cycle equation, exclude all nontrivial cycles
...
Mathlib4 library.  The formalization comprises approximately 2800
lines across 17 files.
...
\begin{tabular}{lll}
  ...
  Cycle elimination & \texttt{CorrectionRatio.lean} &
    $\Delta_3 = 0$: contradiction; $\Delta_3 = 1$: $\{1,2,4\}$; \\
  & & $\Delta_3 \ge 2$: via Baker--Steiner sorry \\
  ...
  K-bound equivalence & \texttt{Drift.lean}, \texttt{Conclusion.lean} &
...
Seven \texttt{sorry} axioms remain, grouped into two categories:
...
\begin{tabular}{clll}
  ...
  6 & \texttt{Baker.lean} &
    \texttt{baker\_no\_balanced\_cycle} &
    Baker--Steiner cycles ($\Delta_3 \ge 2$) \\
  7 & \texttt{Drift.lean} &
...
Item~6 applies Baker's theorem to exclude non-trivial cycles with
$\Delta_3 \ge 2$ and period $p = 3\Delta_3$ (the balanced case),
using Steiner-type analysis of the cycle equation combined with
computational verification for small~$\Delta_3$.  This sorry was
previously in \texttt{CorrectionRatio.lean} as
\texttt{no\_cycle\_equality\_case}; it has been restated in pure
arithmetic terms in \texttt{Baker.lean}, making
\texttt{CorrectionRatio.lean} sorry-free.
Item~7 is the K-bound: the sole \texttt{sorry} on the
...
K-bound (via the $1 \to 4 \to 2 \to 1$ cycle), so the K-bound is
formally equivalent to the Collatz conjecture
(\texttt{nu3\_linear\_bound\_iff\_reaches}).
The Baker--Feldman bridge (Section~\ref{sec:tunnel-eval}) identifies a
route to proving this sorry from Baker's theorem via the structural
pure-even sieve; the missing ingredient is a visitation frequency bound.
...
at some step $t$---has $v_2(3 a_t + 1) \ge 2$, where $a_t$ is the
iterate at step~$t$.  Equivalently, the trajectory is forced to
undergo at least two consecutive halvings at every visit.
\end{definition}

The condition $v_2(3m + 1) \ge 2$ is equivalent to $m \equiv 1 \pmod{4}$:
one checks $3 \cdot 1 + 1 = 4$, and in general $3m + 1 \equiv 4
\pmod{8}$ when $m \equiv 1 \pmod{4}$.  So after an odd step at such
an iterate, the trajectory divides by at least~$4$ before the next
odd step can occur, producing two even steps where only one was
guaranteed by the golden mean shift.
...
$a_t \equiv 1 \pmod{4}$, the cell is structurally pure-even: every
trajectory visiting it undergoes at least two halvings, regardless of
the starting value.
...
any trajectory entering one is forced to take extra even steps,
...
\item[\textup{(ii)}] For any $n \ge 1$ and any step $t$ such that
  ...
  the iterate $a_t$ satisfies $v_2(3 a_t + 1) \ge 2$, so the
  trajectory undergoes at least two consecutive halvings.
...
\item \emph{Structural pure-even cells occupy the Baker gap.}\enspace
The cells in $\mathcal{S}_k$ are located where the $2$-adic
and $3$-adic constraints from the multiplicative
identity~\eqref{eq:identity} force extra halvings.  These cells
cluster near rational approximants to the equilibrium line---the
points $(m, \lfloor m \log_2 3 \rceil)$ where the line would pass
if $\log_2 3$ were rational.  Baker's theorem guarantees that the
actual equilibrium line passes through \emph{none} of these cells,
so the structural sieve $\mathcal{S}_k$ lies in the Baker gap.

\item \emph{The trajectory cannot ride the equilibrium.}\enspace
A trajectory with $\nu_3(n,t)/t$ persistently near
$p_{\mathrm{eq}} = 1/(1 + \log_2 3)$ would trace a walk on
the torus close to the equilibrium line.  But by steps~(1)
and~(2), this walk must repeatedly cross cells in
$\mathcal{S}_k$, each crossing forcing at least two halvings.
Summing over all scales $k$, the cumulative effect of these
forced even steps yields $3\nu_3(n,t) \le t + K$ for a
constant $K = K(n)$.
\end{enumerate}

\noindent
If this bridge can be made fully rigorous---specifically, if one can
prove that every infinite trajectory visits $\mathcal{S}_k$-cells
with frequency bounded below by a positive constant independent
of~$n$---then \texttt{nu3\_linear\_bound} becomes a theorem rather
than an axiom.  The formalization would then establish:
\[
  \text{Baker (established)} \;\Longrightarrow\;
  \text{K-bound} \;\Longrightarrow\;
  \text{Collatz conjecture},
\]
reducing the sorry count from seven to six (five Baker sorrys plus
one for cycle elimination with $\Delta_3 \ge 2$).
