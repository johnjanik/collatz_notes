...
6\,400~lines of Lean~4 across 29~files with four explicit
\texttt{sorry} gaps and four external axioms (Baker~1966,
Rhin~1987, Hercher~2024, Weyl~1916), all identified and categorized.
\end{abstract}
...
\subsection{Remaining gaps: four \texttt{sorry} declarations and four axioms}

Four \texttt{sorry} declarations and four external axioms remain,
...
0$~\cite{Baker1966}.  Item~1 is the Schwarz-type extrapolation
in the Gel'fond--Schneider chain that feeds into axiom~A1.
The two other steps in the Gel'fond--Schneider
...
Item~2 (\texttt{steiner\_cycle\_large}) handles cycles with
$\Delta_3 \ge 80$, the frontier beyond Hercher's computational
verification.  The $\Delta_3 \le 79$ case is fully proved
(Theorem~\ref{thm:steiner-hercher}), using axiom~A1 via
Baker cell separation and axiom~A3 via Hercher's theorem.
\texttt{CycleElimination.lean} and \texttt{CorrectionRatio.lean} are
now sorry-free.

Item~3 is the K-bound: the sole \texttt{sorry} on the critical path
...
Item~4 (\texttt{finite\_residence\_bound}) offers an alternative path
to the K-bound via the Diophantine Repeller.  The decomposition is:
Hensel attrition (proved, \texttt{HenselAttrition.lean}) $+$ Baker
cell separation (proved, reduces to axiom~A1) $+$ finite residence
...
external axiom.  Axiom~A3 (Hercher's verification that no nontrivial
...
route to proving item~3 from axiom~A1 via the structural pure-even
...
axiom~A4 yields equidistribution of $\{n \log_2 3\}$, which combined
