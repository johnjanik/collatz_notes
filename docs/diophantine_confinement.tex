...
5\,800~lines of Lean~4 across 27~files with seven explicit
\texttt{sorry} gaps and two external axioms (Rhin~1987,
Hercher~2024), all identified and categorized.
\end{abstract}
...
5\,800~lines across 27~files, building in 3\,103~jobs.
...
\subsection{Remaining gaps: seven \texttt{sorry} declarations and two axioms}

Seven \texttt{sorry} declarations and two external axioms remain,
...
\begin{tabular}{clll}
  ...
  2--4 & \texttt{GrowthEstimates.lean} &
    \texttt{schwarz\_vanishing\_bound}, &
    Gel'fond--Schneider \\
  & & \texttt{jensen\_zero\_count}, & chain \\
  & & \texttt{polynomial\_zero\_estimate} & \\
  5 & \texttt{SteinerCycle.lean} &
    \texttt{steiner\_cycle\_large} &
    Cycles, $\Delta_3 \ge 80$ \\
  6 & \texttt{Drift.lean} &
    \texttt{nu3\_linear\_bound} &
    K-bound (critical path) \\
  7 & \texttt{DiophantineRepeller.lean} &
...
\max(|m|, |n|)^{1+\delta}$.  Items~2--4 form the Gel'fond--Schneider
proof chain that feeds into item~1: a Schwarz-type extrapolation
(item~2), a Jensen zero-counting argument (item~3), and a polynomial
zero estimate via Vandermonde determinants (item~4).  The
infrastructure for item~4 (injectivity of $2^a \cdot 3^b$ and the
Mathlib Vandermonde API) is in place; the remaining gap is reindexing
between \texttt{Fin}~types and range sums.

Item~5 (\texttt{steiner\_cycle\_large}) handles cycles with
...
Item~6 is the K-bound: the sole \texttt{sorry} on the critical path
...
Item~7 (\texttt{finite\_residence\_bound}) offers an alternative path
...
(this sorry) $\to$ K-bound.  The gap is equidistribution on the
...
route to proving item~6 from item~1 via the structural pure-even
