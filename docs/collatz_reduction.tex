...
\begin{itemize}
  ...
  \item The $E_{10}$ Coxeter matrix and Lehmer's polynomial (\texttt{Lehmer.lean}).
\end{itemize}
...
\subsection{Connection to the Lehmer conjecture and Salem numbers}

The E$_8$ lattice, whose root system is fully formalized in
\texttt{E8Lattice.lean}, is the starting point of a chain that connects
the Collatz problem to several other major open problems in number theory.
The E-series of Coxeter groups,
\[
  E_6 \subset E_7 \subset E_8 \subset E_9 = \widetilde{E}_8 \subset
  E_{10} = T(2,3,7),
\]
extends from finite ($E_6, E_7, E_8$) through affine ($E_9$) to hyperbolic
($E_{10}$).  The Dynkin diagram of $E_{10}$ is the tree $T(2,3,7)$ with
three arms of lengths $2$, $3$, and $7$ emanating from a branch node,
giving $10$ nodes total.
...
\begin{conjecture}[Lehmer, 1933~\cite{Lehmer1933}]
  There exists $c > 0$ such that $M(P) \geq 1 + c$ for every monic,
  irreducible, non-cyclotomic polynomial $P \in \ZZ[x]$.  The conjectured
...
enumerates monic reciprocal polynomials, applies a Graeffe root-squaring
filter to rapidly reject candidates with large Mahler measure, and computes
exact values for survivors.  At degree~$10$ with coefficient bound~$10$, the
sieve searches $4.08 \times 10^6$ polynomials in $0.05$ seconds on $24$ threads
and correctly identifies~$L$ as the unique Salem polynomial with smallest Mahler
measure, recovering $M(L) = 1.17628081826$ to~$12$ significant figures.

The connection to our framework is through the $E_{10}$ Cartan matrix, which
we formalize in \texttt{Lehmer.lean}.  The Cartan matrix $C$ has diagonal
entries $C_{ii} = 2$ and off-diagonal entries $C_{ij} = -1$ for adjacent nodes
in the $T(2,3,7)$ diagram ($9$ edges, $18$ directed adjacencies).  We verify
by \texttt{native\_decide}:
\begin{itemize}
  \item $\mathrm{tr}(C) = 20$, $\mathrm{tr}(C^2) = 58$;
  \item $L$ is reciprocal, monic, with $L(1) = -1$ and $L(-1) = 1$
    (so $L$ has no linear factors);
  \item The plastic number satisfies $1.324 < \theta_0 < 1.325$
    (certifying Smyth's bound to $3$ decimal places).
\end{itemize}
The formalization contains $14$ proved theorems (all by \texttt{native\_decide})
and $3$ axioms: Smyth's bound (deep analytic result), irreducibility of~$L$
(computational), and the Mahler measure bound $1.17 < M(L) < 1.18$ (numerical).

The deeper significance of the $E$-series connection is that the spectral
radius of the $E_{10}$ Coxeter element, Lehmer's number, may represent a
universal obstruction in dynamical systems related to the interaction of
multiplication by~$2$ and~$3$.  The spectral gap $\gamma = 2/3$ for $\times 3$
on $2$-adic partitions (Section~\ref{sec:spectral}) and the Lehmer number
$\lambda_0 \approx 1.176$ are both controlled by the arithmetic of $\log_2 3$,
and both arise from root systems in the same $E$-series.  Whether this
connection can be made precise remains an open problem.
...
M.~J.~Mossinghoff, G.~Rhin, and Q.~Wu, Minimal Mahler measures,
...
\bibitem{Smyth1971}
C.~J.~Smyth, On the product of the conjugates outside the unit circle of an
algebraic integer, \emph{Bull.\ London Math.\ Soc.} \textbf{3} (1971),
169--175.
