...
\begin{empirical}[Branch fraction progression]\label{obs:branch-frac}
    ...
    \end{tabular}
    \end{center}
    All levels $k \leq 108$ are fully saturated (100\% branch cells)
    at $N = 10^{10}$.
    Above $k = 108$, the branch fraction drops sharply as empty cells
    dominate.  For $k \geq 144$, the branch cell count is approximately
    stable across resolutions (${\sim}16{,}371$ at $N = 10^{10}$, within
    ${\sim}1\%$ across $k$), consistent with the support stabilisation
    observed in Section~\ref{sec:profinite-compat}.  The count grows
    with~$N$ (see Remark~\ref{rem:branch-convergence}).
\end{empirical}
...
\begin{empirical}[Singular cells are almost pure-even]\label{obs:singular-podd}
    ...
    $p_{\mathrm{odd}} < 0.3324$.  These are torus positions where
...
\label{obs:frozen-branch}
    At $N = 10^{10}$, the number of branch cells is approximately
    stable across resolutions $k \geq 144$: $k = 144$ has $16{,}371$
    branch cells and $k = 729$ has $16{,}577$ (within ${\sim}1\%$).
    At $k = 729$ this gives $16{,}577$ branch cells of $531{,}441$
    total (branch fraction $3.1\%$).  The branch locus
    has \emph{finite complexity}: the set of trajectory signatures
    producing branching behaviour is approximately captured by
    $k \approx 144$, and larger moduli embed essentially the same set
    in a sparser grid.
\end{observation}

\begin{remark}[Branch count convergence with $N$]
\label{rem:branch-convergence}
    The ``frozen'' property refers to approximate stability across $k$
    (resolution-independence), not convergence in~$N$ (trajectory count).
    As $N$ increases, rare trajectories visit previously unoccupied
    cells with both parities, converting pure cells to branch cells.
    The branch count at $k = 144$ grows substantially with~$N$:
    $9{,}415$ ($N = 10^8$) $\to$ $13{,}688$ ($N = 10^9$) $\to$
    $16{,}371$ ($N = 10^{10}$), an increase of $+74\%$ from $10^8$
    to $10^{10}$.  The branch count has \textbf{not converged} at
    $N = 10^{10}$.  Nevertheless, the qualitative
    structure---approximately resolution-independent branch count,
...
\label{obs:foliation-depletion}
    ...
    $22\times$ \emph{depleted} on the unstable foliation (enrichment
    $0.045$)---branch cells actively avoid the unstable foliation lines.
    The stable foliation shows a milder depletion (enrichment $0.426$,
    a $2.3\times$ depletion).
\end{observation}

\begin{observation}[Diagonal band structure at $k = 729$]
\label{obs:diagonal-band}
    At $N = 10^{10}$, $19{,}025$ non-empty cells occupy $3.6\%$ of the
    $729 \times 729$ torus.  All non-empty cells lie on a single
    coherent diagonal band of width ${\sim}20$~cells, aligned at slope
    $\approx \log_3 2$.  Within this band:
    \begin{itemize}
        \item Branch cells (gold) occupy the core ($16{,}577$),
        \item Pure-even cells ($893$) flank the branch cells on both
              sides, forming ``laminar tunnel walls,''
        \item Pure-odd cells: $0$ (eliminated at $N = 10^{10}$;
              formerly $41$ at $N = 10^9$).
    \end{itemize}
...
\begin{keyinsight}[Diophantine quality controls foliation geometry]
    ...
    \begin{enumerate}[label=(\roman*)]
        ...
        (error $7.2 \times 10^{-5}$), only $3.6\%$ of cells are
        non-empty (at $N = 10^{10}$).
...
\label{obs:oo-verified}
    At $N = 10^{10}$ ($2.27 \times 10^{12}$ total steps), the transition
...
\label{obs:trans-fractions}
    At $N = 10^{10}$, the global transition fractions are
    $\mathrm{ee} : \mathrm{eo} : \mathrm{oe} = 0.3345 : 0.3316 : 0.3339$,
    nearly equal thirds.  The slight asymmetry reflects the departure
    of $p_{\mathrm{odd}} \approx 0.3324$ from the equilibrium value
...
\label{obs:successor}
    ...
    the trajectory lands after the odd step.  At $N = 10^{10}$
    ($17{,}470$ odd-visited cells):
    \begin{center}
    \begin{tabular}{lrr}
    \toprule
    ...
    pure-even tunnel walls of the foliation shadow.  The 5.1\% of
...
\label{res:branch-locus}
    ...
    computed from up to $10^{10}$ trajectories ($2.27 \times 10^{12}$
    steps) with per-cell parity transition tracking, reveals:
    \begin{enumerate}[label=(\roman*)]
        \item \textbf{Universal branching at saturation.}  All levels
        $k \leq 108$ have 100\% branch cells at $N = 10^{10}$.  Above
        $k = 108$, the branch count is approximately stable across
        resolutions (${\sim}16{,}371$ at $k = 144$, ${\sim}16{,}577$ at
        $k = 729$, within ${\sim}1\%$).  The count grows with~$N$ and has
        not converged (see Remark~\ref{rem:branch-convergence}).
        ...
        branch cells are $22\times$ \emph{depleted} on the unstable foliation
        (enrichment $0.045$)---a complete reversal.
        ...
        $\mathrm{oo} = 0$ at every cell ($2.27 \times 10^{12}$ transitions
        verified).  The successors of odd-visited cells at $k = 729$ are
        never pure-odd (94.9\% branch, 5.1\% pure-even), directly linking
...
\begin{remark}[Implementation]
    ...
    ${\sim}100$~MB.  The definitive $N = 10^{10}$ run completed in
    $19.5$~hours, processing $2.27 \times 10^{12}$~steps
    (${\sim}32 \times 10^6$~steps/s).  Per-cell data is written to CSV
